...
export class MCPController {
  ...
  private setupRoutes(): void {
    ...
    this.router.post('/servers/disconnect', (req, res, next) =>
      this.disconnectServer(req, res, next)
    );
   ...
   */
  private async listServerTools(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { serverId } = req.params;
      const client = this.clientManager.getClient(serverId);
   ...
   * Get workflow status
   */
  private async getWorkflowStatus(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
