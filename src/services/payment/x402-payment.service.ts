...
export class X402PaymentService extends EventEmitter {
  ...
  private constructor() {
    ...
    this.networkId = process.env.POLYGON_NETWORK_ID
      ? parseInt(process.env.POLYGON_NETWORK_ID)
      : 137; // Polygon mainnet
  ...
  getUserPaymentRecords(userId: string): PaymentRecord[] {
    return Array.from(this.paymentRecords.values()).filter((r) => r.userId === userId);
  }
   ...
   */
  private async queryBlockchainTransaction(txHash: string): Promise<BlockchainTransaction | null> {
