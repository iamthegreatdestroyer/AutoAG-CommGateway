...
const CONFIG = {
  ...
  HEALTH_CHECK_INTERVAL_MINUTES: parseInt(
    process.env.SERVER_HEALTH_CHECK_INTERVAL_MINUTES || '15',
    10
  ),
...
export class ServerRegistryService extends EventEmitter {
   ...
   */
  public async deleteServer(serverId: string, requesterId: string, reason: string): Promise<void> {
    const server = this.servers.get(serverId);
    if (!server) {
      throw new ServerNotFoundError(serverId);
    }
  ...
  public async searchServers(query: SearchQuery): Promise<SearchResult> {
    ...
    let serverList = Array.from(this.servers.values()).filter(
      (s) => s.status === ServerStatus.ACTIVE
    );
    ...
    if (query.tags && query.tags.length > 0) {
      serverList = serverList.filter((s) => query.tags!.some((tag) => s.tags.includes(tag)));
    }
  ...
  public async getTrendingServers(
    ...
    // Get all active servers
    const servers = Array.from(this.servers.values()).filter(
      (s) => s.status === ServerStatus.ACTIVE
    );
  ...
  public async getTopRatedServers(limit: number = 10): Promise<MCPServer[]> {
    ...
    const servers = Array.from(this.servers.values())
      .filter(
        (s) => s.status === ServerStatus.ACTIVE && s.statistics.totalReviews >= 5 // Minimum reviews for credibility
      )
      .sort((a, b) => {
   ...
   */
  public async updateServerStatus(serverId: string, status: ServerStatus): Promise<void> {
    const server = this.servers.get(serverId);
    if (!server) {
      throw new ServerNotFoundError(serverId);
    }
  ...
  public async validateServerEndpoint(endpoint: string): Promise<ValidationResult> {
    try {
      ...
      const mcpVersion =
        response.headers['x-mcp-version'] ||
  ...
  public resetForTesting(): void {
    ...
    this.servers.clear();
    this.publisherServers.clear();
    this.categoryIndex.clear();
    this.tagIndex.clear();
    this.cache.clear();

    // Re-initialize category index
    Object.values(ServerCategory).forEach((category) => {
      this.categoryIndex.set(category as ServerCategory, new Set());
    });
  }
