...
describe('RatingService', () => {
  ...
  describe('getRatings', () => {
    ...
    it('should filter by minimum stars', async () => {
      const filtered = await service.getRatings(
        'server-123',
        { minStars: 4 },
        { page: 1, limit: 10 }
      );
    ...
    it('should filter by review presence', async () => {
      const withReviews = await service.getRatings(
        'server-123',
        { hasReview: true },
        { page: 1, limit: 10 }
      );
    ...
    it('should sort by helpful votes', async () => {
      ...
      const sorted = await service.getRatings(
        'server-123',
        {},
        { page: 1, limit: 10, sortBy: 'helpful' }
      );
  ...
  describe('Moderation', () => {
    ...
    it('should enforce report rate limiting (10 per day)', async () => {
      ...
      await expect(service.reportRating(rating11.id, 'reporter-123', 'Spam')).rejects.toThrow(
        RateLimitError
      );
    });
  ...
  describe('markRatingHelpful', () => {
    ...
    it('should prevent duplicate helpful votes', async () => {
      ...
      await expect(service.markRatingHelpful(testRating.id, 'voter-123')).rejects.toThrow(
        ValidationError
      );
      await expect(service.markRatingHelpful(testRating.id, 'voter-123')).rejects.toThrow(
