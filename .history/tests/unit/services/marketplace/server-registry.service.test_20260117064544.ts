...
describe('ServerRegistryService', () => {
  ...
  beforeEach(() => {
    ...
    // Clear any existing servers (reset state)
    const stats = service.getServiceStats();
    // Since we can't directly access private members, we'll work with a fresh instance
  ...
  describe('registerServer', () => {
    ...
    it('should reject registration with name too short', async () => {
      ...
      await expect(service.registerServer(invalidRegistration))
        .rejects
    ...
    it('should reject registration with unreachable endpoint', async () => {
      ...
      await expect(service.registerServer(validRegistration))
        .rejects
        .toThrow(InvalidServerError);
    });
    ...
    it('should reject registration when publisher has reached limit', async () => {
      ...
      // 51st should fail
      await expect(service.registerServer({
        ...validRegistration,
        name: 'Server 51',
  ...
  describe('updateServer', () => {
    ...
    it('should update server successfully', async () => {
      ...
      expect(updated.updatedAt.getTime()).toBeGreaterThan(testServer.createdAt.getTime());
    });
    ...
    it('should reject update for non-existent server', async () => {
      await expect(service.updateServer('non-existent', {}, publisherId))
        .rejects
        .toThrow(ServerNotFoundError);
    });

    it('should reject update from non-owner', async () => {
      await expect(service.updateServer(testServer.id, {}, 'wrong-publisher'))
        .rejects
        .toThrow(UnauthorizedError);
    });

    it('should reject update with invalid name', async () => {
      await expect(service.updateServer(testServer.id, { name: 'AB' }, publisherId))
        .rejects
        .toThrow(InvalidServerError);
    });
  ...
  describe('deleteServer', () => {
    ...
    it('should reject deletion of non-existent server', async () => {
      await expect(service.deleteServer('non-existent', publisherId, 'Test'))
        .rejects
        .toThrow(ServerNotFoundError);
    });

    it('should reject deletion by non-owner', async () => {
      await expect(service.deleteServer(testServer.id, 'wrong-publisher', 'Test'))
        .rejects
        .toThrow(UnauthorizedError);
    });
  });
  ...
  describe('searchServers', () => {
    ...
    it('should sort by rating', async () => {
      ...
      for (let i = 1; i < result.servers.length; i++) {
        expect(result.servers[i - 1].statistics.averageRating)
          .toBeGreaterThanOrEqual(result.servers[i].statistics.averageRating);
      }
    });
    ...
    it('should sort by popularity', async () => {
      ...
      for (let i = 1; i < result.servers.length; i++) {
        expect(result.servers[i - 1].statistics.totalInvocations)
          .toBeGreaterThanOrEqual(result.servers[i].statistics.totalInvocations);
      }
    });
    ...
    it('should sort by newest', async () => {
      ...
      for (let i = 1; i < result.servers.length; i++) {
        expect(result.servers[i - 1].createdAt.getTime())
          .toBeGreaterThanOrEqual(result.servers[i].createdAt.getTime());
      }
    });
  ...
  describe('getServerById', () => {
    ...
    it('should throw error for non-existent server', async () => {
      await expect(service.getServerById('non-existent'))
        .rejects
        .toThrow(ServerNotFoundError);
    });
  });
  ...
  describe('getTopRatedServers', () => {
    ...
    it('should return top rated servers', async () => {
      ...
      for (let i = 1; i < topRated.length; i++) {
        expect(topRated[i - 1].statistics.averageRating)
          .toBeGreaterThanOrEqual(topRated[i].statistics.averageRating);
      }
    });
  ...
  describe('getPublisherServers', () => {
    ...
    it('should sort by creation date descending', async () => {
      ...
      for (let i = 1; i < servers.length; i++) {
        expect(servers[i - 1].createdAt.getTime())
          .toBeGreaterThanOrEqual(servers[i].createdAt.getTime());
      }
    });
  ...
  describe('performHealthCheck', () => {
    ...
    it('should throw error for non-existent server', async () => {
      await expect(service.performHealthCheck('non-existent'))
        .rejects
        .toThrow(ServerNotFoundError);
    });
  });
  ...
  describe('updateServerStatistics', () => {
    ...
    it('should throw error for non-existent server', async () => {
      await expect(service.updateServerStatistics('non-existent', {}))
        .rejects
        .toThrow(ServerNotFoundError);
    });
  });
