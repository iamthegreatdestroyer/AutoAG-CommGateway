...
describe('CommissionService', () => {
  ...
  beforeEach(() => {
    ...
    // Clear service state
    (service as any).commissions.clear();
    (service as any).payoutBatches.clear();
    (service as any).affiliateLinks.clear();
    (service as any).publisherCommissions.clear();
    (service as any).serverCommissions.clear();
    (service as any).affiliateCommissions.clear();
  ...
  describe('recordCommission', () => {
    ...
    it('should generate unique commission IDs', async () => {
      const commission1 = await service.recordCommission('payment-1', 'server-1', 'publisher-1', 1000n);
      const commission2 = await service.recordCommission('payment-2', 'server-2', 'publisher-2', 2000n);
  ...
  describe('Commission Lifecycle', () => {
    ...
    beforeEach(async () => {
      commission = await service.recordCommission('payment-123', 'server-456', 'publisher-789', 1000n);
    });
    ...
    it('should throw error for non-existent commission', async () => {
      await expect(service.getCommission('non-existent-id')).rejects.toThrow(CommissionNotFoundError);
    });

    it('should filter commissions by status', async () => {
      await service.confirmCommission(commission.id);

      const pending = await service.getPublisherCommissions('publisher-789', CommissionStatus.PENDING);
      const confirmed = await service.getPublisherCommissions('publisher-789', CommissionStatus.CONFIRMED);
  ...
  describe('createPayoutBatch', () => {
    it('should create payout batch above threshold', async () => {
      ...
      const commission1 = await service.recordCommission('payment-1', 'server-1', 'publisher-789', 100000n);
      const commission2 = await service.recordCommission('payment-2', 'server-2', 'publisher-789', 100000n);

      await service.confirmCommission(commission1.id);
      await service.confirmCommission(commission2.id);

      const batch = await service.createPayoutBatch('publisher-789', [commission1.id, commission2.id]);
    ...
    it('should reject batch below threshold', async () => {
      ...
      const commission = await service.recordCommission('payment-1', 'server-1', 'publisher-789', 50000n);
      await service.confirmCommission(commission.id);

      await expect(
        service.createPayoutBatch('publisher-789', [commission.id])
    ...
    it('should validate all commissions are confirmed', async () => {
      const commission = await service.recordCommission('payment-1', 'server-1', 'publisher-789', 150000n);
      // Don't confirm
    ...
    it('should group commissions by publisher', async () => {
      const pub1Commission = await service.recordCommission('payment-1', 'server-1', 'publisher-A', 150000n);
      const pub2Commission = await service.recordCommission('payment-2', 'server-2', 'publisher-B', 150000n);
    ...
    it('should generate unique batch IDs', async () => {
      const commission1 = await service.recordCommission('payment-1', 'server-1', 'publisher-A', 150000n);
      const commission2 = await service.recordCommission('payment-2', 'server-2', 'publisher-B', 150000n);
  ...
  describe('scheduleAutomatedPayouts', () => {
    it('should schedule payouts for eligible publishers', async () => {
      ...
      const commission1 = await service.recordCommission('payment-1', 'server-1', 'publisher-A', 100000n);
      const commission2 = await service.recordCommission('payment-2', 'server-2', 'publisher-A', 100000n);
    ...
    it('should skip publishers below threshold', async () => {
      ...
      const commission = await service.recordCommission('payment-1', 'server-1', 'publisher-B', 50000n);
      await service.confirmCommission(commission.id);
    ...
    it('should handle multiple publishers', async () => {
      ...
      const commA1 = await service.recordCommission('payment-1', 'server-1', 'publisher-A', 100000n);
      const commA2 = await service.recordCommission('payment-2', 'server-2', 'publisher-A', 100000n);
    ...
    it('should only process confirmed commissions', async () => {
      const confirmed = await service.recordCommission('payment-1', 'server-1', 'publisher-A', 100000n);
      const pending = await service.recordCommission('payment-2', 'server-2', 'publisher-A', 100000n);
  ...
  describe('createAffiliateLink', () => {
    ...
    it('should reject duplicate affiliate codes', async () => {
      ...
      await expect(
        service.createAffiliateLink('server-456', 'affiliate-2', code)
      ).rejects.toThrow(CommissionError);
      await expect(
        service.createAffiliateLink('server-456', 'affiliate-2', code)
  ...
  describe('getAffiliatePerformance', () => {
    ...
    it('should aggregate affiliate metrics', async () => {
      ...
      const comm1 = await service.recordCommission('payment-1', 'server-1', 'publisher-1', 1000n, 'affiliate-456');
      const comm2 = await service.recordCommission('payment-2', 'server-2', 'publisher-2', 2000n, 'affiliate-456');
    ...
    it('should identify top performing servers', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const comm = await service.recordCommission(`payment-1-${i}`, 'server-1', 'publisher-1', 1000n, 'affiliate-456');
        await service.trackAffiliateConversion('CODE1', comm.id);
      }

      // Server 2: 10 conversions
      for (let i = 0; i < 10; i++) {
        const comm = await service.recordCommission(`payment-2-${i}`, 'server-2', 'publisher-2', 1000n, 'affiliate-456');
  ...
  describe('Commission Queries', () => {
    ...
    it('should get commissions by status', async () => {
      const pending = await service.getPublisherCommissions('publisher-1', CommissionStatus.PENDING);
      const confirmed = await service.getPublisherCommissions('publisher-1', CommissionStatus.CONFIRMED);
