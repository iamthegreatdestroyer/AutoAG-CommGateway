...
describe('RatingService', () => {
  ...
  beforeEach(() => {
    ...
    // Clear service state
    (service as any).ratings.clear();
    (service as any).toolRatings.clear();
    (service as any).userRatings.clear();
    (service as any).helpfulVotes.clear();
    (service as any).ratingReports.clear();
  ...
  describe('submitRating', () => {
    ...
    it('should reject invalid star values (less than 1)', async () => {
      ...
      await expect(service.submitRating(invalidSubmission)).rejects.toThrow(ValidationError);
      await expect(service.submitRating(invalidSubmission)).rejects.toThrow('Stars must be between 1 and 5');
    });
    ...
    it('should enforce review length limit (1000 characters)', async () => {
      ...
      await expect(service.submitRating(invalidSubmission)).rejects.toThrow(ValidationError);
      await expect(service.submitRating(invalidSubmission)).rejects.toThrow('Review must not exceed 1000 characters');
    });

    it('should prevent duplicate ratings from same user', async () => {
      await service.submitRating(validSubmission);

      await expect(service.submitRating(validSubmission)).rejects.toThrow(ValidationError);
      await expect(service.submitRating(validSubmission)).rejects.toThrow('already rated this tool');
    });
  ...
  describe('updateRating', () => {
    ...
    it('should update own rating successfully', async () => {
      ...
      expect(updated.updatedAt.getTime()).toBeGreaterThan(existingRating.createdAt.getTime());
    });
    ...
    it('should enforce review length limit on update', async () => {
      ...
      await expect(
        service.updateRating(existingRating.id, updates, 'user-456')
      ).rejects.toThrow(ValidationError);
    });
  });
  ...
  describe('deleteRating', () => {
    ...
    it('should delete own rating', async () => {
      ...
      await expect(service.getRating(existingRating.id)).rejects.toThrow(RatingNotFoundError);
    ...
    it('should reject unauthorized deletion', async () => {
      await expect(
        service.deleteRating(existingRating.id, 'different-user')
      ).rejects.toThrow(UnauthorizedError);

      // Rating should still exist
      const rating = await service.getRating(existingRating.id);
      expect(rating).toBeDefined();
    });

    it('should throw error for non-existent rating', async () => {
      await expect(
        service.deleteRating('non-existent-id', 'user-456')
  ...
  describe('Reputation Algorithm', () => {
    ...
    it('should calculate distribution correctly', async () => {
      await service.submitRating({ toolId: 'server-123', userId: 'user-1', stars: 5, verified: true });
      await service.submitRating({ toolId: 'server-123', userId: 'user-2', stars: 5, verified: true });
      await service.submitRating({ toolId: 'server-123', userId: 'user-3', stars: 4, verified: true });
      await service.submitRating({ toolId: 'server-123', userId: 'user-4', stars: 3, verified: true });
      await service.submitRating({ toolId: 'server-123', userId: 'user-5', stars: 1, verified: true });
  ...
  describe('getRatings', () => {
    ...
    it('should get ratings with pagination', async () => {
      const page1 = await service.getRatings('server-123', {}, { page: 1, limit: 2 });
      ...
      const page2 = await service.getRatings('server-123', {}, { page: 2, limit: 2 });
      expect(page2.ratings).toHaveLength(2);
      expect(page2.hasMore).toBe(true);
    });

    it('should filter by minimum stars', async () => {
      const filtered = await service.getRatings('server-123', { minStars: 4 }, { page: 1, limit: 10 });
    ...
    it('should filter by review presence', async () => {
      const withReviews = await service.getRatings('server-123', { hasReview: true }, { page: 1, limit: 10 });
    ...
    it('should sort by helpful votes', async () => {
      const ratings = await service.getRatings('server-123', {}, { page: 1, limit: 10 });
      const rating1 = ratings.ratings[0];
      const rating2 = ratings.ratings[1];

      // Mark first rating helpful by multiple users
      await service.markRatingHelpful(rating1.id, 'user-a');
      await service.markRatingHelpful(rating1.id, 'user-b');
      await service.markRatingHelpful(rating1.id, 'user-c');

      // Mark second rating helpful by one user
      await service.markRatingHelpful(rating2.id, 'user-d');

      const sorted = await service.getRatings('server-123', {}, { page: 1, limit: 10, sortBy: 'helpful' });
  ...
  describe('Moderation', () => {
    ...
    it('should report rating with valid reason', async () => {
      await service.reportRating(testRating.id, 'reporter-123', 'Spam or inappropriate content');

      const rating = await service.getRating(testRating.id);
    ...
    it('should enforce report rate limiting (10 per day)', async () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        await service.reportRating(rating.id, 'reporter-123', 'Spam');
      }
      ...
      await expect(
        service.reportRating(rating11.id, 'reporter-123', 'Spam')
      ).rejects.toThrow(RateLimitError);
    });

    it('should allow moderator to delete rating', async () => {
      await service.moderateRating(testRating.id, 'moderator-789', 'delete', 'Spam content removed');

      await expect(service.getRating(testRating.id)).rejects.toThrow(RatingNotFoundError);
    });

    it('should reject non-moderator deletion attempts', async () => {
      await expect(
        service.moderateRating(testRating.id, 'regular-user', 'delete', 'Notes')
      ).rejects.toThrow(UnauthorizedError);
    });

    it('should allow moderator to approve reported rating', async () => {
      await service.reportRating(testRating.id, 'reporter-123', 'Spam');
  ...
  describe('markRatingHelpful', () => {
    ...
    it('should mark rating as helpful', async () => {
      await service.markRatingHelpful(testRating.id, 'voter-123');
    ...
    it('should prevent duplicate helpful votes', async () => {
      ...
      await expect(
        service.markRatingHelpful(testRating.id, 'voter-123')
      ).rejects.toThrow(ValidationError);
      await expect(
        service.markRatingHelpful(testRating.id, 'voter-123')
      ).rejects.toThrow('already marked this rating as helpful');
    });

    it('should track multiple users voting', async () => {
      await service.markRatingHelpful(testRating.id, 'voter-1');
      await service.markRatingHelpful(testRating.id, 'voter-2');
      await service.markRatingHelpful(testRating.id, 'voter-3');
  ...
  describe('ServerRegistry Integration', () => {
    ...
    it('should handle ServerRegistry update failures gracefully', async () => {
      mockServerRegistry.updateServerStatistics.mockRejectedValueOnce(new Error('Registry unavailable'));
  ...
  describe('Event Emissions', () => {
    it('should emit rating:submitted event', async () => {
      ...
      expect((event as any).rating.stars).toBe(5);
    });
    ...
    it('should emit rating:helpful event', async () => {
      ...
      await service.markRatingHelpful(rating.id, 'voter-123');
  ...
  describe('getServiceStats', () => {
    it('should return aggregate statistics', async () => {
      await service.submitRating({ toolId: 'server-1', userId: 'user-1', stars: 5, verified: true });
      await service.submitRating({ toolId: 'server-2', userId: 'user-2', stars: 4, verified: true });
      await service.submitRating({ toolId: 'server-1', userId: 'user-3', stars: 3, verified: true, review: 'Good' });

      const stats = service.getServiceStats();

      expect(stats.totalRatings).toBe(3);
      expect(stats.totalReviews).toBe(1);
      expect(stats.averageStars).toBeGreaterThan(3);
