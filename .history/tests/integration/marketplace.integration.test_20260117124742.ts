...
import { serverRegistryService, ServerCategory, ServerStatus } from '../../src/services/marketplace/server-registry.service';
import { ratingService } from '../../src/services/marketplace/rating.service';
import { commissionService, CommissionStatus } from '../../src/services/marketplace/commission.service';
...
describe('Marketplace Integration Tests', () => {
  ...
  describe('Complete Server Lifecycle Workflow', () => {
    it('should handle full workflow: register → rate → commission → payout', async () => {
      ...
      expect(processedBatch.blockchainTxHash).toBeDefined();
      ...
      expect(revenueReport.totalCommissions).toBeGreaterThan(0);
      expect(revenueReport.totalPaidOut).toBeGreaterThan(0);
      expect(revenueReport.transactionsCount).toBe(11);
    });
  });
  ...
  describe('Multi-Server Publisher Dashboard', () => {
    it('should aggregate data across multiple servers', async () => {
      ...
      const commissions = await commissionService.getPublisherCommissions(publisherId, CommissionStatus.CONFIRMED);
      ...
      expect(revenue.commissionCount).toBe(3);
    });
  });
  ...
  describe('Cache Behavior Across Services', () => {
    it('should invalidate caches when data changes', async () => {
      ...
      expect(revenue.commissionCount).toBe(1);
      expect(revenue.grossRevenue).toBeGreaterThan(0);
    });
  });
