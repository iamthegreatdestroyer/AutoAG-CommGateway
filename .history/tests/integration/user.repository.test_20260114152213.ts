...
describe('UserRepository', () => {
  ...
  describe('createUser', () => {
    ...
    it('should fail to create user with duplicate email', async () => {
      ...
      await expect(db.users.createUser({ ...userData, username: 'duplicate2' })).rejects.toThrow();
    });
  });
  ...
  describe('updateWalletBalance', () => {
    it('should increment wallet balance', async () => {
      ...
      await db.users.updateWalletBalance(user.id, 25.5);

      const updatedUser = await db.users.findById(user.id);
      expect(Number(updatedUser?.walletBalance)).toBe(75.5);
    });
