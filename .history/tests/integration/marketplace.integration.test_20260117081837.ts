...
describe('Marketplace Integration Tests', () => {
  ...
  describe('Complete Server Lifecycle Workflow', () => {
    it('should handle full workflow: register → rate → commission → payout', async () => {
      ...
      const serverRatings = await ratingService.getRatings(server.id, {}, { page: 1, limit: 10 });
      expect(serverRatings.items).toHaveLength(1);
      expect(serverRatings.items[0].id).toBe(rating.id);
      ...
      expect(commission.platformFee).toBe(500n); // 5%
      expect(commission.publisherAmount).toBe(9500n); // 95%
      ...
      expect(confirmedCommission.status).toBe('confirmed');
  ...
  describe('Event Propagation Across Services', () => {
    it('should emit and receive events across service boundaries', async () => {
      ...
      serverRegistryService.on('server:registered', (e) => events.push({ service: 'registry', event: 'registered', data: e }));
      ratingService.on('rating:submitted', (e) => events.push({ service: 'rating', event: 'submitted', data: e }));
      commissionService.on('commission:recorded', (e) => events.push({ service: 'commission', event: 'recorded', data: e }));
