...
export class EscrowService extends EventEmitter {
  ...
  async batchSettlement(): Promise<SettlementLog> {
    ...
    try {
      // Calculate totals
      const totalAmount = recordsToSettle.reduce((sum, r) => sum + BigInt(r.amount), BigInt(0));
  ...
  calculateFees(amount: string): string {
    ...
    const fee = (bigAmount * BigInt(Math.floor(this.config.feePercentage * 100))) / BigInt(10000); // Convert percentage to basis points
    return fee.toString();
  }
  ...
  getUserEscrow(userAddress: string): EscrowRecord[] {
    return Array.from(this.escrowRecords.values()).filter((r) => r.userAddress === userAddress);
  }
   ...
   */
  private async processSettlementBatch(log: SettlementLog, records: EscrowRecord[]): Promise<void> {
