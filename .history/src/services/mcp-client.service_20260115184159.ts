...
export class MCPServerClient {
  ...
  async invokeTool(request: ToolInvocationRequest): Promise<ToolInvocationResponse> {
    ...
    // Create invocation tracking record
    ...
    try {
      ...
      if (!tool) {
        const error = 'Tool not found on server';
        this.invocationTracker.recordFailure(correlationId, error, 0);

        throw new MCPToolInvocationError(request.toolId, this.serverId, error);
      }
      ...
      this.invocationTracker.recordSuccess(correlationId, result, 0);
      ...
      // Record failure invocation
      const invocationRecord = this.invocationTracker.getInvocation(correlationId);
      if (invocationRecord?.status === 'pending') {
        this.invocationTracker.recordFailure(correlationId, error instanceof Error ? error.message : String(error), 0);
      }

      // Initiate rollback if this was a critical operation
      if (request.invokedBy && error.code !== 'ECONNABORTED') {
        const rollback = this.rollbackService.initiateRollback(
          correlationId,
          `Tool invocation failed: ${request.toolId}`,
          request.invokedBy,
          {
            toolId: request.toolId,
            serverId: this.serverId,
        ...
        if (rollback) {
          this.logger.info(`[MCP] Initiated rollback: ${rollback.rollbackId}`, {
            correlationId,
            toolId: request.toolId,
          });
        }
