...
export class RatingService extends EventEmitter {
  ...
  public async updateRating(
    ...
    if (updates.review !== undefined && updates.review.length > 0) {
      if (updates.review.length < CONFIG.MIN_REVIEW_LENGTH) {
        throw new InvalidRatingError(`Review must be at least ${CONFIG.MIN_REVIEW_LENGTH} characters`);
      }
      if (updates.review.length > CONFIG.MAX_REVIEW_LENGTH) {
        throw new InvalidRatingError(`Review must not exceed ${CONFIG.MAX_REVIEW_LENGTH} characters`);
      }
    }
  ...
  public async getReportedRatings(
    ...
    let reported = Array.from(this.ratings.values())
      .filter(r => r.reported && !r.moderatorReviewed);
