...
export class CommissionService extends EventEmitter {
   ...
   */
  public async processPayoutBatch(batchId: string): Promise<{ success: boolean; transactionHash?: string; error?: string }> {
  ...
  public async createAffiliateLink(
    ...
    const affiliateLinks = Array.from(this.affiliateLinks.values())
      .filter(l => l.affiliateId === affiliateId);
    if (affiliateLinks.length >= CONFIG.MAX_AFFILIATE_LINKS_PER_USER) {
      throw new AffiliateError(`Maximum ${CONFIG.MAX_AFFILIATE_LINKS_PER_USER} affiliate links per user`);
    }
   ...
   */
  public async recordAffiliateConversion(
    code: string,
    userId: string
  ): Promise<void> {
    const linkId = this.affiliateCodeMap.get(code);
  ...
  private generateAffiliateCode(): string {
    let code: string;
    do {
      code = Math.random().toString(36).substring(2, 2 + CONFIG.AFFILIATE_CODE_LENGTH).toUpperCase();
