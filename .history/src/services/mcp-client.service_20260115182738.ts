...
export class MCPServerClient {
  ...
  async connect(connectionRequest: ServerConnectionRequest): Promise<ServerConnectionResponse> {
    try {
      this.logger.info(`[MCP] Connecting to server: ${connectionRequest.serverName} (${connectionRequest.serverUrl})`);
  ...
  async invokeTool(request: ToolInvocationRequest): Promise<ToolInvocationResponse> {
    ...
    try {
      ...
      const invocationError = error instanceof MCPError ? error : new MCPToolInvocationError(
        request.toolId,
        this.serverId,
        error.message,
        error,
